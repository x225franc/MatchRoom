<template>
  <div class="min-h-screen bg-white">
    ...
    <div class="space-y-4 px-4 pb-10 pt-4" v-if="!selectedHotel">
      ...
      <div v-else v-for="hotel in filteredHotels" :key="hotel.id"
        ...
        <div class="relative">
          ...
          <!-- Status Icon (Alert or Pending) -->
          <span v-if="hotel.alert" class="absolute top-2 right-2 z-20 p-1 bg-white/80 rounded-full shadow">
          ...
          <span v-else class="absolute top-2 right-2 z-20 p-1 bg-white/80 rounded-full shadow">
    ...
    <!-- Detail View (Negotiation/Accept/Reject) -->
    <div v-else class="px-4 py-6 space-y-4">
      <!-- Hotel Image and Price in Detail View -->
      <div class="relative">
...
const activeFilter = ref(filters[0]); // Default filter
const allHotels = ref([]); // Holds all reservations fetched from API
const selectedHotel = ref(null); // Currently selected hotel for detail view
const isLoading = ref(true); // Loading state for initial data fetch
const isProcessingPayment = ref(false); // Loading state for payment redirection
const stripeLoadingError = ref(null); // Error during Stripe initialization
let stripe = null; // Stripe instance

// --- Computed Properties ---
// Filters the hotels based on the active tab
...
function setActiveFilter(filterName) {
  ...
  selectedHotel.value = null; // Go back to list view when changing filter
}

// Selects a hotel to show details
...
function onImageError(event) {
  event.target.src = '/placeholder.jpg'; // Replace with your placeholder image path
}
...
async function fetchReservations() {
  ...
  try {
    ...
    allHotels.value = [
      { id: 'hotel_1', name: 'Hotel de Paris', image: "/rooms/example.jpg", rating: 4.5, alert: false, originalPrice: 230, agreedPrice: 220, status: 'Acceptées', distance: '3,4km', address: 'Paris, adresse 1' },
      { id: 'hotel_2', name: 'Hotel Magnifique', image: "/rooms/example2.jpg", rating: 5, alert: true, originalPrice: 250, agreedPrice: 240, status: 'Acceptées', distance: '1,2km', address: 'Paris, adresse 2' },
      ...
      { id: 'hotel_5', name: 'Hotel Vue Mer', image: "/rooms/example2.jpg", rating: 4, alert: false, originalPrice: 300, agreedPrice: 280, status: 'Acceptées', distance: '10km', address: 'Nice, bord de mer' }, // Example with missing image
    ];
    // --- END MOCK DATA ---
...
async function handleSwipe(direction) {
  ...
  if (direction === "left") {
    ...
    // TODO: Call API to update reservation status to 'Refus'
    // Example: await api.updateReservationStatus(selectedHotel.value.id, 'Refus');

    // Update local status (optional, depends on API response handling)
    const hotelInList = allHotels.value.find(h => h.id === selectedHotel.value.id);
    if (hotelInList) hotelInList.status = 'Refus';

    selectedHotel.value = null; // Go back to list
  } else if (direction === "right") {
    // --- Logic for ACCEPT (Proceed to Payment) ---
    ...
    isProcessingPayment.value = true; // Show loading state on button

    try {
      // Redirect to Stripe Checkout (Client-Only)

      // === THAY ĐỔI QUAN TRỌNG ===
      // 1. Thay 'YOUR_PRICE_ID_HERE' bằng Price ID thật từ Stripe Dashboard
      const priceId =  'price_1RHo5zPqIGsIWpx4lGdn1X1S'; // Ví dụ: 'price_1PG...'
      ...
      const { error } = await stripe.redirectToCheckout({
        lineItems: [{
          // price_data: { ... }, // <<=== XÓA HOẶC COMMENT KHỐI NÀY
          price: priceId,       // <<=== SỬ DỤNG PRICE ID
          quantity: quantity,   // <<=== SỬ DỤNG QUANTITY ĐÃ TÍNH
        }],
        mode: 'payment',
...
<style scoped>
/* Add subtle transition for background color on filter buttons */
button {
  transition: background-color 0.2s ease-in-out, color 0.2s ease-in-out, transform 0.2s ease-in-out;
}

/* Optional: Add styles for placeholder image */
img[src$="/placeholder.jpg"] {
  /* Example: Add a background color or pattern */
  background-color: #f0f0f0;
}

/* Styles for loading spinner (if needed elsewhere) */
.animate-spin {
