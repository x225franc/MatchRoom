<template>
  <div class="min-h-screen bg-white dark:bg-neutral-900">
    <div class="flex justify-around py-4 border-b dark:border-neutral-700">
      <button
        ...
        :class="[
          ...
          activeFilter === filter
            ...
            : 'border border-black text-black hover:bg-gray-100 dark:border-white dark:text-white dark:hover:bg-neutral-800',
        ]"
      >
        {{ filter }}
      </button>
    ...
    <div class="space-y-4 px-4 pb-10 pt-4" v-if="!selectedHotel">
      <div
        v-if="isLoading"
        class="text-center text-gray-500 mt-10 dark:text-gray-400"
      >
        Chargement...
      </div>
      <div
      ...
      <div
        ...
        :key="hotel.id"
        class="bg-white rounded-xl shadow-md overflow-hidden cursor-pointer hover:shadow-lg transition-shadow dark:bg-neutral-800"
        @click="selectHotel(hotel)"
      >
        <div class="relative">
          <div
            class="absolute top-0 left-0 w-full h-20 bg-gradient-to-b from-black/60 to-transparent z-10 pointer-events-none"
          ></div>
          <img
            ...
            :alt="hotel.name"
          />
          <span class="absolute top-2 right-2 z-20 p-1 rounded-full">
            <template v-if="hotel.status === 'Acceptées'">
              <svg
                width="32"
                height="32"
                viewBox="0 0 32 32"
            ...
            <template v-else-if="hotel.status === 'Refus'">
              <svg
                width="32"
                height="32"
                viewBox="0 0 32 32"
            ...
            <template v-else-if="hotel.alert">
              <svg
                width="32"
                height="32"
                viewBox="0 0 28 28"
        ...
        <div class="p-4">
          <h3
            class="font-semibold text-lg truncate dark:text-white"
            :title="hotel.name"
          >
            {{ hotel.name }}
          </h3>
          <div class="flex items-center text-yellow-400 text-sm mb-2">
            <span v-for="i in Math.floor(hotel.rating || 0)" :key="`full-${i}`"
              >★</span
            >
            <span v-if="hotel.rating % 1 !== 0">½</span>
            <span
              v-for="i in Math.floor(5 - (hotel.rating || 0))"
              :key="`empty-${i}`"
              class="text-gray-300 dark:text-gray-600"
              >★</span
            >
            <span class="text-gray-500 text-xs ml-2 dark:text-gray-400"
              >({{ hotel.rating || "N/A" }})</span
            >
          </div>
          <div
            class="text-gray-500 text-sm flex items-center space-x-2 dark:text-gray-400"
          >
    ...
    <div v-else class="px-4 py-6 space-y-4">
      <div class="relative">
        <img
          :src="selectedHotel.image || '/placeholder.jpg'"
          @error="onImageError"
      ...
      <div class="flex items-center text-yellow-400 text-sm mb-2">
        <span
          v-for="i in Math.floor(selectedHotel.rating || 0)"
          :key="`full-detail-${i}`"
          >★</span
        >
        <span v-if="selectedHotel.rating % 1 !== 0">½</span>
        <span
          v-for="i in Math.floor(5 - (selectedHotel.rating || 0))"
          :key="`empty-detail-${i}`"
          class="text-gray-300"
          >★</span
        >
        <span class="text-gray-500 text-xs ml-2"
          >({{ selectedHotel.rating || "N/A" }})</span
        >
      </div>
      <div
        class="text-gray-600 text-sm flex items-center space-x-2 dark:text-gray-400"
      >
        <span class="flex items-center"
          ...
          {{ selectedHotel.distance || "N/A" }}</span
        >
      ...
      <div class="border-t pt-4 mt-4 space-y-3">
        ...
        <div class="text-center mt-4">
          <button
            class="text-sm underline text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"
    ...
    allHotels.value = [
      {
        ...
        status: "En cours",
        distance: "3,4km",
        address: "Paris, 32 Rue de Rivoli",
      },
      {
        id: "hotel_2",
        ...
        status: "Acceptées",
        distance: "1,2km",
        address: "Paris, 64 Rue de Ranelagh",
      },
      {
        id: "hotel_3",
        name: "Petit Hotel",
        ...
        status: "Refus",
        distance: "5,0km",
        address: "Paris, 242 Faubourg Saint-Antoine",
      },
      {
        id: "hotel_4",
        ...
        status: "En cours",
        distance: "0.5km",
        address: "Paris, 6 Place Vandetta",
      },
      {
        id: "hotel_5",
        name: "Hotel Vue Mer",
        ...
        status: "Acceptées",
        distance: "10km",
        address: "Nice, Promenade des Anglais",
      },
    ];
...
async function handleSwipe(direction) {
  ...
  if (direction === "left") {
    ...
    const hotelInList = allHotels.value.find(
      (h) => h.id === selectedHotel.value.id
    );
    ...
    console.log(
      `Accepting offer for ${selectedHotel.value.name}, proceeding to payment...`
    );
    ...
    const hotelImage = selectedHotel.value.image
      ? [window.location.origin + selectedHotel.value.image]
      : [];
    ...
    try {
      ...
      if (isNaN(quantity) || quantity <= 0) {
        alert(
          `Giá hoặc số lượng không hợp lệ (quantity=${quantity}). Kiểm tra agreedPrice.`
        );
        isProcessingPayment.value = false;
        return;
      }
      ...
      console.log(
        `Redirecting to Stripe with Price ID: ${priceId}, Quantity: ${quantity}`
      );
