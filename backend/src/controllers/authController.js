...
import { generateJWT, verify2FA as verify2FAToken } from '../utils/authUtils.js';
...
const setup2FA = async (req, res) => {
  try {
    ...
    const secret = speakeasy.generateSecret({ name: `TinderHotels:${user.email}` });
    await User.update2FASecret(user.id, secret.base32);
    res.json({ qrCode: secret.otpauth_url });
  } catch (err) {
    res.status(500).json({ message: 'Erreur lors de la configuration 2FA', error: err.message });
  }
};
...
const verify2FA = async (req, res) => {
  ...
  try {
    ...
    const verified = verify2FA(user.two_factor_secret, token);
    if (!verified) {
    ...
    res.status(500).json({ message: 'Erreur lors de la vérification 2FA', error: err.message });
  }
};
