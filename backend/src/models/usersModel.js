...
const User = {
  create: async ({ email, password, name }) => {
    ...
    `;
    const { rows } = await pool.query(query, [email, password, name, new Date()]);
    return rows[0];
  },
  ...
  clearSession: async (id) => {
    await pool.query('UPDATE users SET session_token = NULL, session_expires_at = NULL WHERE id = $1', [id]);
  },
};
